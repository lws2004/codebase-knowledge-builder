...
class MermaidRegenerator:
    ...
    def regenerate_mermaid_content(self, content: str, context: Optional[str] = None) -> str:
        ...
        def regenerate_block(match):
            ...
            # 重新生成图表
            regenerated_content = self._regenerate_single_mermaid(
                mermaid_content, errors, context
            )
...
1. 保持图表的原始含义和结构
2. 修复所有语法错误
3. 确保符合 Mermaid 最新语法规范
4. 支持中文字符显示
5. 使用简洁清晰的节点标签
6. 避免使用特殊符号和复杂嵌套

常见修复规则：
- 移除节点标签中的括号、引号、大括号等特殊符号
- 修复嵌套方括号问题
- 确保箭头语法正确
- 移除行尾分号
- 确保图表类型声明正确
...
def regenerate_mermaid_in_content(
    content: str, 
    llm_client=None, 
    context: Optional[str] = None
) -> str:
...
def validate_and_regenerate_mermaid(
    content: str, 
    llm_client=None, 
    context: Optional[str] = None
) -> Tuple[str, bool]:
