...
class CodeParser:
    ...
    def parse_repo(self, max_files: int = 1000, max_file_size: int = 1024 * 1024) -> Dict[str, Any]:
        ...
        try:
                ...
                for file in files:
                    ...
                    try:
                        ...
                        if file_size > max_file_size:
                            log_and_notify(f"文件过大，跳过: {rel_path} ({file_size} 字节)", "warning")
                        ...
                        if file_info:
                            ...
                            if lang:
                                ...
                                result["language_stats"][lang]["line_count"] += file_info.get("line_count", 0)
                            
                            # 更新目录信息
                            if rel_root:
                                if rel_root in result["directories"]:
            ...
            # 更新目录结构（添加子目录信息）
            for dir_path, dir_info in result["directories"].items():
                parent_dir = os.path.dirname(dir_path)
                if parent_dir and parent_dir in result["directories"]:
                    result["directories"][parent_dir]["subdirectories"].append(dir_path)
            ...
            for pattern in import_patterns:
                ...
                for match in matches:
                    if len(match.groups()) == 1:
                        ...
                        for item in items:
                            imports.append(f"from {module} import {item.strip()}")
    ...
    def extract_functions(self, content: str, language: str) -> List[Dict[str, Any]]:
        ...
        if language == 'python':
            ...
            func_patterns = [
                r'(?:function\s+([^\s\(]+))?\s*\(([^\)]*)\)\s*{',  # 普通函数
                r'(?:const|let|var)\s+([^\s=]+)\s*=\s*(?:function)?\s*\(([^\)]*)\)\s*(?:=>)?\s*{',  # 函数表达式
                r'(?:const|let|var)\s+([^\s=]+)\s*=\s*\(([^\)]*)\)\s*=>'  # 箭头函数
            ]
