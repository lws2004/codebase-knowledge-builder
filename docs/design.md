...
- **自动修复**：检测到错误时自动调用LLM重新生成符合语法的图表
- **多层回退**：CLI验证失败时回退到基于规则的语法检查
- **性能优化**：进程池隔离，异步处理，避免阻塞主线程

### 📈 7维度质量评估体系
- **全面评估**：完整性、准确性、可读性、格式化、可视化、教学价值、实用性
- **智能优化**：低于阈值自动触发重新生成，提供具体改进建议
- **迭代改进**：支持多轮优化，持续提升文档质量

### ⚡ 高性能并行处理
- **异步并行流程**：支持异步节点的并行执行，显著提升效率
- **智能并发控制**：可配置并发数，信号量机制，异常隔离
- **批处理优化**：大型代码库分批处理，避免资源过载

### 🔧 模块化架构增强
- **节点式设计**：基于PocketFlow框架，易于扩展和定制
- **流程连接器**：灵活的节点连接机制，支持复杂工作流
- **错误恢复**：全局错误处理框架，自动恢复和降级策略
...
10. **`validate_mermaid_syntax(mermaid_content: str)`** (`utils/mermaid_validator.py`)
    - _输入_: Mermaid 图表内容 (str)
    - _输出_: (是否有效: bool, 错误列表: List[str])
    - _必要性_: 验证Mermaid图表语法的正确性，确保可正常渲染。
    - _实现策略_:
      - **优先策略**: 使用mermaid-cli进行真实渲染验证
      - **回退策略**: 基于规则的语法检查
      - **性能优化**: 进程池隔离，异步处理
    - _错误处理_: CLI不可用时自动回退，超时处理，资源清理
    ...
    async def validate_mermaid_syntax(mermaid_content: str) -> Tuple[bool, List[str]]:
        """验证 Mermaid 图表语法"""
        # 1. 清理和预处理内容
        # 2. 尝试使用 mermaid-cli 进行真实验证
        # 3. 失败时回退到基于规则的简单验证
        # 4. 返回验证结果和错误信息
    ```

11. **`validate_and_regenerate_mermaid(content: str, llm_client, context: str)`** (`utils/mermaid_regenerator.py`)
    - _输入_: 文档内容 (str), LLM客户端, 上下文描述 (str)
    - _输出_: (修复后内容: str, 是否修复: bool)
    - _必要性_: 检测文档中的Mermaid图表并自动修复语法错误。
    - _核心功能_:
      - **图表提取**: 从文档中提取所有Mermaid代码块
      - **语法验证**: 逐个验证图表语法
      - **智能修复**: 调用LLM重新生成错误的图表
      - **内容替换**: 将修复后的图表替换回原文档
    - _错误处理_: LLM调用失败时保留原图表，记录错误信息

    ```python
    # 核心实现概要
    def validate_and_regenerate_mermaid(content: str, llm_client, context: str) -> Tuple[str, bool]:
        """验证并重新生成文档中的 Mermaid 图表"""
        # 1. 提取文档中的所有 Mermaid 代码块
        # 2. 逐个验证图表语法
        # 3. 对于有错误的图表，调用 LLM 重新生成
        # 4. 替换原文档中的错误图表
        # 5. 返回修复后的内容和修复标志
    ```
...
12.  **`chunk_text(text, chunk_size=1000, overlap=200, smart_chunking=True)`** (`utils/rag_utils.py`)
    - _输入_: 文本 (str), 块大小 (int), 重叠大小 (int), 智能分块标志 (bool)
    - _输出_: 文本块列表 (list of str)
    - _必要性_: 将代码和文档分割成适合嵌入和检索的块。
    - _智能分块_: 如果启用，尊重代码和文档的自然边界（如函数、类、段落）。

13. **`get_embedding(text, model='default', batch=False)`** (`utils/embedding.py`)
...
14. **`vector_search(query_embedding, index, top_k=5, similarity_threshold=0.7)`** (`utils/vector_db.py`)
...
15. **`create_vector_index(embeddings, metadata=None, index_type='flat')`** (`utils/vector_db.py`)
...
16. **`split_content_into_files(content_dict, output_dir, file_structure=None, repo_structure=None, just_the_docs_compatible=True)`** (`utils/formatter.py`)
...
##### 5. `MermaidValidationNode` 🆕

- **目的**: 验证和修复生成文档中的Mermaid图表，确保图表语法正确且可正常渲染。
- **输入**: `shared["output_files"]`, `shared["llm_config"]`
- **输出**: `shared["mermaid_validation_result"]`
- **核心功能**:
  - **多层验证机制**: 优先使用mermaid-cli进行真实渲染验证，回退到规则检查
  - **自动修复**: 检测到语法错误时调用LLM重新生成符合规范的图表
...
- **目的**: 支持异步节点的并行执行，显著提升大型代码库的处理效率。
- **输入**: 多个异步节点或流程的列表
- **输出**: 所有并行任务的执行结果
- **核心功能**:
  - **异步并行执行**: 同时运行多个异步节点，充分利用I/O等待时间
  - **智能节点包装**: 自动将同步节点包装为异步节点
  - **结果合并**: 智能合并所有并行任务的结果到共享存储
