...
from src.nodes import InputNode, PrepareRepoNode, AnalyzeRepoFlow
...
def main():
    ...
    parser.add_argument("--repo-url", type=str, default="https://github.com/octocat/Hello-World.git", help="Git 仓库 URL")
    ...
    if result.get("success", False):
        print("\n分析仓库流程完成:")
        print(f"- 代码结构: {result['code_structure'].get('file_count', 0)} 个文件, {result['code_structure'].get('directory_count', 0)} 个目录")
        print(f"- 核心模块: {len(result['core_modules'].get('modules', []))} 个")
        print(f"- 历史分析: {result['history_analysis'].get('commit_count', 0)} 个提交, {result['history_analysis'].get('contributor_count', 0)} 个贡献者")
