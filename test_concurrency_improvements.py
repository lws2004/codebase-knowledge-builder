...
from src.utils.performance_monitor import get_performance_monitor, TaskMonitoringContext
...
async def test_concurrency_manager():
    ...
    # 使用并发管理器处理任务
    results = await manager.process_batch_async(
        items=tasks,
        process_func=simulate_llm_call,
        max_concurrency=8
    )
...
async def test_load_balancer():
    ...
    # 使用负载均衡器处理任务
    results = await balancer.execute_batch(
        task_func=simulate_file_processing,
        task_args=files,
        timeout=5.0
    )
